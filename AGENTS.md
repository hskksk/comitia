...
- エンジンプラグインは Claude Code、OpenCode（`enginebay` 経由）、`fake` 以外を足さない（設計 03 §6）。Cursor / Gemini はまだ。足す・同梱する・ホストする前に [設計 11](docs/design/11-engine-vendor-terms.md) の灰色ゾーンを読む。
