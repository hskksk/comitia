...
| 直近の出来事 | ダッシュボードの Event 一覧（M26 で人が読む活動へ置換予定） | ツールが無い。M21 の briefing 通知と役割が重なる |
...
6. **意図的に隠しているものは開けない。** チャットログ・トレース（登録オーナーだけ）、他人のメモリ・非公開メモ、資格情報、招待トークン。自分のエージェントのメモリは登録オーナーが人間 REST で読む（[設計 19](19-owner-agent-memory.md)）。エージェントのツールからは開けない
...
| 判断キュー本体 | 人間の注意。エージェントにキュー画面を複製しない（原則 7）。スレッド単位の到達フラグは [設計 21](21-judgment-queue-reach.md) |
| 非ブロッキング Inbox | 同上。`unclaimed_decided` と PR 付き `open_threads` で作業の位置は既にある |
| 直近の活動 | M26 は人間ダッシュボード専用。M21 の briefing 通知と二重になるため、専用 `list_events` / `list_activity` は作らない |
| 他人のメモリ・非公開メモ | 可視性なし。登録オーナーが自分のエージェントのメモリを読むのは人間 REST（[設計 19](19-owner-agent-memory.md)）。ツールでは開けない |
| 資格・招待 | 秘密 |
...
- 判断キュー / Inbox のエージェント複製、専用 `list_events`。スレッド単位の到達フラグ（`queuedForHumanJudgment`）は [設計 21](21-judgment-queue-reach.md) M30
- ダッシュボードにスレッドテンプレ・スキルのヒーローを足すこと
- M26 のダッシュボード活動をエージェント向けツールへ複製すること
- スキル本文を朝のパックへ全部載せる閾値
...
| 判断キュー（争点＋候補の人間画面） | 出さない | — | — | — | — | 到達だけ [設計 21](21-judgment-queue-reach.md) `queuedForHumanJudgment`。一覧は複製しない | |
| 非ブロッキング Inbox | 出さない | — | — | — | — | PR と局面で位置は分かる | |
| 直近の活動 | 出さない | — | — | — | — | — | 人間は M26。agent は M21 の通知 |
| 他人のメモリ・非公開メモ | 出さない | — | — | — | — | — | 登録オーナーの人間 REST は [設計 19](19-owner-agent-memory.md) |
