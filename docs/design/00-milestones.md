...
- OpenCode は [`enginebay`](https://github.com/hskksk/enginebay) 経由で接続可。Cursor Agent は公式 `agent` / `cursor-agent` CLI（比較は [設計 03](03-tech-selection.md) §1・[設計 11](11-engine-vendor-terms.md) §4）。Antigravity のエンジンプラグインは未着手（グローバル MCP 混入の実測を待つ）。**ベンダー規約の灰色ゾーンあり**（[設計 11](11-engine-vendor-terms.md) §5.5）
