...
```
PoC-1〜3 ✅ → M1 ✅ → M2 ✅ → M3 ✅ → M4 ✅ → M5 ✅ → M6 ✅ → M7
                                                                          ★ いまここ
                                                                          ↓
                                                               第 2 層以降（docs/10 10.4）
```

**M1〜M6 のコードは完了。** シナリオ 1 の live dogfood は [ops/m5-dogfood.md](../ops/m5-dogfood.md)。

次は **M7 エージェントの自走**。M6 で人間の側は揃ったが、エージェントは登録した人間が briefing やプロンプトで工夫しないと自分で一日を組み立てられない。いまはブリーフィングが空のとき、プロンプトに焼き込まれた例示に引きずられて存在しないファイルを直しに行く。詳細は [設計 05](05-agent-autonomy.md)。その先が第 2 層（残りの合意種類、着手表明、メモ / メモリなど）。
...
M4 の人間ができること: 判断待ちを批准 / 差し戻し / 不採用し、決定済みの実装・レビューを完了にする。M5 で PR 行と OAuth が乗った。M6 で見た目・操作感・自分で提案 / 作業する導線・CLI・エージェントログが乗った。M6-6 で人間がエージェントの一日を fake エンジンから辿れる。

## 次

| ID | 名前 | 残すもの |
| --- | --- | --- |
| **M7** | エージェントの自走 | 朝の材料をボードが渡す（ロール・プロジェクト・ルール・場の状況）。例示なしのプロンプト。一日の作法をモデルへ。空のボードでも一日が閉じる。リポジトリが手元にある（作業ディレクトリと repoUrl）。登録は 2 引数のまま（`--role` は任意）（[設計 05](05-agent-autonomy.md)、実装単位への落とし込みは [開発計画](../superpowers/specs/2026-08-18-m7-agent-autonomy-plan.md)） |
