...
次は **M13 アカウント・複数プロジェクト・運転シェル**。1 プロジェクト・人間一人のままでは comitia 自身をボードで運転できない。詳細は [設計 07](07-accounts-and-shell.md)。**M14（エージェントの GitHub 資格）は M13-2 以降と並列可**（[設計 08](08-agent-github-credentials.md)）。M13 のあと、第 2 層のドメイン（M8〜M12、[設計 06](06-layer2.md)）をボード上の提案として進める。
...
M14 は M5 の installation と M3 のエージェントベアラがあれば足りる。M13-2 のシェルには依存しない。
