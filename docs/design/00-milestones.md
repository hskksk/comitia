...
```
PoC-1〜3 ✅ → M1〜M7 ✅ → M8〜M12 ✅ → M13 ✅ → M14 ✅ → M15 ✅ → M16 ✅ → M30
                                                                              ★ いまここ
                                                                 ↓
                                                    M17（M30-2 のあと）
                                                    M18
...
次は **M30 共有物の構造**（[#148](https://github.com/hskksk/comitia/issues/148)。[設計 21](21-shared-artifacts.md)）。改正の正本と置換。**M17 改善提案の効果検証** は改正の定義が M30 に依存するため、M30-2 のあとに切る。横断の **M20（エージェント可観測性）** は M15 完了を受けて M20-1 から着手可（[設計 10](10-agent-observability.md)）。**第 4 層（通知）** は [設計 12](12-layer4-notifications.md) の M21 として M16〜M20 と並列可。**実装スレッドの作業局面** は [設計 13](13-implementation-work-phase.md) の M22 として並列可。**fake 操作台** は [設計 15](15-fake-console.md) の M23 として並列可。ボードの所有・参照は [設計 14](14-board-domain-model.md)（マイルストーンではない）。M24 は欠番。第 3 層の残りは [設計 09](09-layer3.md)。swarm は第 3 層バックログのまま番号を振らない。
...
**共有物の構造**（[設計 21](21-shared-artifacts.md)）。#148。kind の最新合意ではなく文書と現行ポインタ。M17 より先。

| ID | 名前 | 残すもの |
| --- | --- | --- |
| **M30-1** | 共有物の構造の設計 | 文書 identity、創設と改正、成立時の置換、スキルは key ごと。コードは入れない |
| **M30-2** | schema + domain | `shared_artifacts`。成立が旧合意を `superseded` にする。現行はポインタ。マイグレーション |
| **M30-3** | MCP / REST / briefing | 起票の文書 id / key。一覧と朝のパックが現行ポインタ。決定ビューの本文 diff |
| **M30-4** | UI / CLI | 創設と改正を画面で分ける。ダッシュボードの現行。例示なし |

**第 3 層**（[設計 09](09-layer3.md)）。改善ループが回ること。

| ID | 名前 | 残すもの |
| --- | --- | --- |
| **M17** | 改善提案の効果検証 | 共有物の改正に期待効果と見直し時期。到来はブリーフィング。キューには入れない。改正の定義は M30。実装は M30-2 のあと |
...
- OpenCode は [`enginebay`](https://github.com/hskksk/enginebay) 経由で接続可。Cursor Agent も enginebay の `cursor-agent` ドライバ経由（比較は [設計 03](03-tech-selection.md) §1・[設計 11](11-engine-vendor-terms.md) §4）。Antigravity のエンジンプラグインは未着手（グローバル MCP 混入の実測を待つ）。**ベンダー規約の灰色ゾーンあり**（[設計 11](11-engine-vendor-terms.md) §5.5）
...
- 同一マイルストーン内の依存するサブタスクは stacked PR にする。設計が未マージなら実装はその設計 PR に積む。手順は [AGENTS.md](../../AGENTS.md)
