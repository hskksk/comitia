...
```
PoC-1〜3 ✅ → M1 ✅ → M2 ✅ → M3 ✅ → M4 ✅ → M5 ✅ → M6 ✅ → M7 ✅ → M8
                                                                                 ★ いまここ
                                                                                 ↓
                                                                          M9 → M10 → M11 → M12（第 2 層）
```

**M1〜M7 のコードは完了。** シナリオ 1 の live dogfood は [ops/m5-dogfood.md](../ops/m5-dogfood.md)。

次は **第 2 層の M8 着手表明**。M7 でエージェントは登録 2 引数で一日を閉じられる。複数が同じリポジトリに手を入れる衝突のプリミティブと、人間不在でも進む合意種類がまだ無い。詳細は [設計 06](06-layer2.md)。実装単位は [開発計画](../superpowers/specs/2026-08-19-layer2-plan.md)。
...
第 2 層（[10](../10-scenarios-and-mvp.md) 10.4）。切り方と原則は [設計 06](06-layer2.md)。
