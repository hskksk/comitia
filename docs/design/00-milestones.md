...
**横断の小さな運転 UX**（[設計 13](13-implementation-work-phase.md)、[設計 15](15-fake-console.md)、[設計 17](17-dashboard-activity.md)）。M16〜M21 と並列可。
...
| **M26-1** | 活動フィード設計 | ダッシュボードの直近は監査ログではなく操作の投影。tick は出さない（[設計 17](17-dashboard-activity.md)） |
| **M26-2** | フィード投影 | `DASHBOARD_FEED_KINDS`。`GET /v1/events` が絞り、題・抜粋・refs を載せる |
| **M26-3** | フィード UI | ダッシュボードが操作・行為者・場・付随を日本語で出す |
