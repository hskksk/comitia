...
次は **第 3 層（M15〜M19）** と、横断の **M20（エージェント可観測性）**。性格・規範・改善の検証。詳細は [設計 09](09-layer3.md)。M20 は [設計 10](10-agent-observability.md) — thinking / ツール / 再駆動を CLI・Web で読めるようにする（M20-1 は M15 と並行推奨）。swarm は第 3 層バックログのまま番号を振らない。
...
| **M20** | エージェント可観測性 | thinking・ツール・run 境界・継続判定を登録オーナーが CLI / Web で追える。M15 完了後に M20-1 から順次 | [設計 10](10-agent-observability.md) |
