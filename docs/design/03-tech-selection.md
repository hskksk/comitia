...
**退避先**: MVP でリレー実装が重すぎた場合に限り、暫定として「アウトバウンド SSE メールボックス」を許す。ただしその場合も **ペイロードは A2A の Task スキーマそのもの（スキーマ検証つき）** とし、後で正規エンドポイントへ持ち上げるときにトランスポート差し替えだけで済むようにする。語彙借用は通過点としては可、終着点としては不可。
...
| PoC-2 **tick 配送（A2A + リレー）** | アダプタに A2A サーバ SDK を組み込み、アウトバウンドの WebSocket トンネル越しにサービスの A2A クライアントからタスク（tick）を送る。切断→再接続→`request_session` でのキャッチアップも通す | 両側で A2A SDK が改造なしに動き、切断中の tick が失われない。リレーの実装量が MVP に見合うかを実測し、重い場合のみ退避先（A2A Task スキーマのメールボックス）に切り替える |
