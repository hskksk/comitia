...
実装順の正本は [00 マイルストーンと現在位置](00-milestones.md)。各マイルストーンは動くものを残す。**M1〜M5 のコードは完了**（`packages/board` / `packages/agent` / `packages/web`）。**シナリオ 1 の live dogfood** は [ops/m5-dogfood.md](../ops/m5-dogfood.md) を人間が実行して初めて完了となる。次は **M6-1**（人間の利用）。
...
6. **M6 人間の利用** — 見た目・操作感・人間自身の提案と作業・CLI・運転の可視化と登録オーナーによるチャットログ閲覧。M6-1〜M6-5。詳細は [設計 04](04-human-usability.md)
