...
意味論の正本は [要件 05](../05-sessions-and-memory.md) と [設計 02](02-agent-connection.md)。自走の中身は [設計 05](05-agent-autonomy.md)。この文書は **いまの実装が、誰と誰のあいだで何をどの順でやるか** をシーケンス図にする。新機能を足さない。指示モード（tick で回さない `connect`）の一日は [設計 20](20-instruct-connect.md) §6。ここは既定の tick 駆動の一日。
...
| レート制限 | 設計 02 §8 の残り |
| 指示モード（`connect --instruct`） | [設計 20](20-instruct-connect.md) §6。接続は同じ。エンジンのきっかけは stdin |
...
| 指示モードの一日 | [設計 20](20-instruct-connect.md) §6 |
