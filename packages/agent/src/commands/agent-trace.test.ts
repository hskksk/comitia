...
describe("agentTraceCommand", () => {
  ...
  it("loads all trace pages before returning", async () => {
    const configDir = await mkdtemp(join(tmpdir(), "comitia-agent-trace-"));
    await saveConfig(configDir, {
      boardUrl: "http://127.0.0.1:9",
      ownerToken: "owner-token",
    ...
    const fetchMock = vi.fn(async (input: string | URL) => {
      ...
      if (url.includes("/v1/sessions/sess-1/trace?afterSeq=0")) {
        return new Response(
          JSON.stringify({
            sessionId: "sess-1",
            hasMore: true,
      ...
      if (url.includes("/v1/sessions/sess-1/trace?afterSeq=1")) {
        return new Response(
          JSON.stringify({
            sessionId: "sess-1",
            hasMore: false,
  ...
  it("polls for new trace entries in follow mode", async () => {
    const configDir = await mkdtemp(join(tmpdir(), "comitia-agent-trace-"));
    await saveConfig(configDir, {
      boardUrl: "http://127.0.0.1:9",
      ownerToken: "owner-token",
