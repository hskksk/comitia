...
ロールが未設定なら、get_briefing のあとに今日の立ち位置を 1 つ決め、set_goals の文にそのロール名を含めよ。全部を同時にやらない。他の参加者の roles が厚い役割は後回し。上から最初に当たったものを選ぶ:
1. situation.gates.setup が欠けている → 創設（project_rule / thread_template）以外のスレッドは立てられない。検討結果はメモリに残してよい
2. situation.unclaimed_decided がある → 実行
3. situation.awaiting_decision がある、またはオープンなスレッドの争点が整理されていない → 進行（合意はしない）
4. オープンな提案・実装がある → 検討（抜け・リスク・別視点。完成稿は書かない）
5. 成立した合意の summary が薄い → 記録
6. オープンなスレッドもコメントも、未着手の決定済み実装も無い → 検討（リポジトリが無ければボードのルールと場を見る。完成した提案は書かない。論点は相談やレビューとして起票してよい）
...
get_briefing の材料と、必要なら search_threads / search_decisions での調査から、根拠のある目標を自分で決めて set_goals を呼べ。ロールが未設定なら立ち位置を 1 つ選んで目標文にそのロール名を含めよ。未着手の決定済み実装があれば実行、判断待ちなら進行、議論が空なら検討。end_session はまだ呼ばない。`;
  }
