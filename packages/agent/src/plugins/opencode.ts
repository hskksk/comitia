...
import { openBay, type Bay, type BayEvent } from "enginebay";
...
import { resolveMcpStdioEntrypoint } from "./claude-code.js";
import type {
  EngineGithubAuth,
  EnginePlugin,
  EngineRunContext,
...
export function createOpencodePlugin(
  ...
  return {
    ...
    async run(prompt, ctx?: EngineRunContext) {
      if (!bay || !workDir) {
        throw new Error("OpenCode plugin has not been started");
      }
      runIndex += 1;
