...
import { getActiveSharedArtifact, getProjectSetup } from "./constitution.js";
...
export async function getProjectSummary(db: Db, projectId: string) {
  ...
  const [queue, inbox, setup, activeProjectRule] = await Promise.all([
      ...
      : await db
          ...
          .where(
            and(
              ...
              inArray(events.actorParticipantId, ids),
            ),
          )
          .orderBy(desc(events.createdAt));
  const lastActionByParticipant = new Map<string, Date>();
  for (const row of actionRows) {
    if (!row.actorParticipantId) continue;
    if (!lastActionByParticipant.has(row.actorParticipantId)) {
      lastActionByParticipant.set(row.actorParticipantId, row.createdAt);
    }
  }
    ...
    people.map(async (person) => {
      ...
      return {
        ...
        openSession: open
          ...
          : null,
        wake,
        lastActionAt: iso(lastActionByParticipant.get(person.id) ?? null),
      };
    }),
  );
}
