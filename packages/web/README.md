...
- PR プレビュー（Railway PR Environment）では Variables 不要で「プレビューに入る」が自動表示（OAuth は非表示）。CLI 用は「CLI 用にトークンをコピー」
- 手動検証用は `COMITIA_BOOTSTRAP_TOKEN`、常駐 staging で止めるなら `COMITIA_AUTO_PREVIEW=0`
